from datetime import datetime
from TaskProject.db import *
from Exceptions.Error import *

def output_menu():
    print('''
    ---------- The task book ----------
    1. Check the tasks
    2. Add a task
    3. Delete a task
    4. Update a task
    0. Exit
    ------------------------------''')

def output_records_tasks():
    tasks, columns = view_tasks()

    print('\n<<<<<<<<<< List of records >>>>>>>>>>')

    for task in range(0, len(tasks)):
        for column in range(0, len(columns)):
            print(f'{columns[column][0].capitalize()}: {tasks[task][column]}')

        print()

    print('<<<<<<<<<<>>>>>>>>>>')

def input_user_for_added_task():
    print('\n<<<<<<<<<< Added record >>>>>>>>>>')

    title = input('Title: ')
    description = input('Description: ')
    due_date = input('Due date: ')

    added_task(title, description, due_date)

def deleting_entry():
    print('\n<<<<<<<<<< Deleting records >>>>>>>>>>')

    output_records_tasks()

    id_task = input('Task ID: ')

    deleted_task(id_task)

def changed_record_tasks():
    print('\n<<<<<<<<<< Changing records >>>>>>>>>>')

    output_records_tasks()

    id_task = input('Enter the task id to change: ')
    title_update = input('Title: ')
    description_update = input('Description: ')
    due_date_update = input('Due date: ')
    status_update = input('Status: ')

    updated_date_update = datetime.now().strftime('%Y-%m-%d %H:%M:%S')

    changed_task(id_task, title_update, description_update, due_date_update, status_update, updated_date_update)

def validate_input_user(user_input):
    if user_input == '0':
        return False
    if user_input == '1':
        output_records_tasks()
        return True
    if user_input == '2':
        input_user_for_added_task()
        return True
    if user_input == '3':
        deleting_entry()
        return True
    if user_input == '4':
        changed_record_tasks()
        return True

    raise InvalidInputError